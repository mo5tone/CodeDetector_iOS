...
extension CodeDetectorPreview: UIGestureRecognizerDelegate {
...
// MARK: - UIView
open class CodeDetectorPreview: UIView {
  public var regionOfInterestStrokeColor = UIColor.lightGray
  public var cornerControlFillColor = UIColor.white
  var videoPreviewLayer: AVCaptureVideoPreviewLayer {
    guard let layer = layer as? AVCaptureVideoPreviewLayer else {
      fatalError("Expected `AVCaptureVideoPreviewLayer` type for layer. Check Preview.layerClass implementation.")
    }
    return layer
  }
  ...
  private var regionOfInterestControlRadius: CGFloat {
    return regionOfInterestControlDiameter / 3.0
  }
  ...
  private lazy var resizeRegionOfInterestGestureRecognizer: UIPanGestureRecognizer = { UIPanGestureRecognizer(target: self, action: #selector(resizeRegionOfInterestWithGestureRecognizer(_:))) }()
...
extension CodeDetectorPreview {
  private func commonInit() {
    ...
    regionOfInterestOutline.strokeColor = regionOfInterestStrokeColor.cgColor
